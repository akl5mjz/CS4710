...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        nextTransitionStates = self.mdp.getTransitionStatesAndProbs(state, action)
        newValues = 0

        for nextState, prob in nextTransitionStates:
            reward = self.mdp.getReward(state, action, nextState)
            discount = self.discount * self.values[nextState]
            Qvalue = prob * (reward + discount)
            newValues += Qvalue
    ...
    def computeActionFromValues(self, state):
        ...
        for action in legalActions:
            ...
            actionQvalues.append((action, Qvalue))
        bestAction = max(actionQvalues, key = lambda x: x[1])[0]
        return bestAction
    ...
    def runValueIteration(self):
        ...
        for _ in range(self.iterations):
            ...
            if not self.mdp.isTerminal(newState):
                bestAction = self.computeActionFromValues(newState)
                QValue = self.computeQValueFromValues(newState, bestAction)
          ...
          for p in predecessors[temp_state]:
            if not self.mdp.isTerminal(p):
              ...
              for action in self.mdp.getPossibleActions(p):
                ...
                values.append(q_value)
              diff = abs(max(values) - self.getValue(p))
              if diff > self.theta:
                pq.update(p, -diff)
